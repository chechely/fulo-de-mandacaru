...
const Mito3 = ({navigation}) => {

    let filtro = mito.filter(mito => mito.Mito === "Você precisa menstruar todo mês para que seu corpo desintoxique");
     console.log(filtro)
  
// Mostra na tela o titulo da Verdade (a mesma coisa que foi digitada)
  
  const titulo =  Object.entries(filtro).map(([key, value]) => {
     return <Text style={styles.titulo} key={key}>{value.Mito}</Text>
   });
  ...
  return(
    <SafeAreaView style={{flex: 1, backgroundColor: '#5A4BB7'}}>
      <ScrollView style={{flex: 1}} endFillColor='white'>
        <Text style={styles.pageTitle}>Mito</Text>
